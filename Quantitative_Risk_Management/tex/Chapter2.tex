...
\subsubsection{General Definitions}
    ...
    Let $(\Omega,\F,\Prob)$ be the probability space on which all random variables are defined. 

\begin{definition}[Loss and Loss Distribution]\ \par
    For a portfolio value process $V(s)$ and a fixed horizon $\Delta \in \Z$, the \emph{loss} over $[s,s+\Delta]$ is
\[
...
\[
    ...
    L_{t+1}:=-(V_{t+1}-V_t),
\]
and the distribution of $L_{t+1}$ is called the \emph{loss distribution}.
\end{definition}

\begin{definition}[Risk Factors]\ \par
The portfolio value is usually represented as a function of time and risk factors, which is usually a $d$-dimensional vector:
\[
    V_t=f(t,Z_t),\qquad
    Z_t=(Z_{t,1},\ldots,Z_{t,d})^\top,
\] and we may define the risk-factor change
\[
    X_{t+1}:=Z_{t+1}-Z_t.
\]
Then we may know
...
\]
and hence once the current portfolio and the current risk factors are fixed, the loss distribution is induced by the distribution of $X_{t+1}$.
\end{definition}

\begin{definition}[Loss Operator]\ \par
...
\[
    L_{t+1}^{\Delta}
    :=-\left(\partial_t f(t,Z_t)+\sum_{i=1}^d \partial_i f(t,Z_t)X_{t+1}^i\right),
\]
with corresponding linearized loss operator
\[
    l_{[t]}^{\Delta}(x)
    :=-\left(\partial_t f(t,Z_t)+\sum_{i=1}^d \partial_i f(t,Z_t)x^i\right).
\]
which is most reliable for short horizons and for portfolios whose value is nearly linear in the risk factors.
\end{proposition}

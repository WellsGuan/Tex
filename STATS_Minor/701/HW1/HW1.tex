...
\begin{aligned}
    b_{-3} + b_{-2} + b_{-1} + b_0 + b_1 + b_2 + b_3&= 1, \\
    -3b_{-3} - 2b_{-2} - b_{-1} + b_1 + 2b_2 + 3b_3&= 0, \\
    9b_{-3} + 4b_{-2} + b_{-1} + b_1 + 4b_2 + 9b_3&= 0\\
\end{aligned}
\]
...
\begin{aligned}
    ...
    &= \mathbb{E}\left(\sum\limits_{i=-2}^2 a_i Z_{t+i}\right)^2 =\sum\limits_{i=-2}^2 a_i^2 \sigma^2 = \dfrac{17}{35}\sigma^2 \\
    \text{Var}V_t &= \mathbb{E}\left(\sum\limits_{i=-3}^3 b_i (m_{t+i} + Z_{t+i})-m_t\right)^2 \\
    &= \mathbb{E}\left(\sum\limits_{i=-3}^3 b_i Z_{t+i}\right)^2 =\sum\limits_{i=-3}^3 b_i^2 \sigma^2 = \dfrac{1}{3}\sigma^2
\end{aligned}
\]
(ii) Notice that
\[
...
\begin{aligned}
    ...
    &= \sigma^2(a_{-1}a_{-2} + a_0a_{-1} + a_1a_0 + a_2a_1) = \dfrac{48}{175}\sigma^2 \\
    \text{Cov}(V_t,V_{t+1}) &= \mathbb{E}\left(\sum\limits_{i=-3}^3 b_iZ_{t+i}\right)\left(\sum\limits_{i=-3}^3 b_iZ_{t+1+i}\right) \\
    &= \sigma^2(b_{-2}b_{-3} + b_{-1}b_{-2} + b_0b_{-1} + b_1b_0 + b_2b_1 + b_3b_2)= \dfrac{12}{49}\sigma^2 \\
\end{aligned}
\]
For $\sigma^2>0$, dividing by the corresponding variances gives
\[
\operatorname{Corr}(U_t,U_{t+1})=\frac{48/175}{17/35}=\frac{48}{85},
\qquad
\operatorname{Corr}(V_t,V_{t+1})=\frac{12/49}{1/3}=\frac{36}{49}.
\]
(iii) The correlation between $V_t$ and $V_{t+1}$ is greater than that between
$U_t$ and $U_{t+1}$, and $V_t$ has the smaller noise variance. Since both filtered
series preserve the same trend $m_t$, we expect $(V_t)_{t\in\mathbb Z}$ to be
smoother in appearance. If $\sigma^2=0$, both series equal $m_t$.
\par
\vspace{0.5em}
...
\[
\operatorname{Cov}(X_t,X_{t+1})=\sigma^2\cos(ct)\sin(c(t+1)).
\]
This is $\sigma^2\sin c$ at $t=0$ and zero at $t=-1$.
...
\end{figure}\par
Data points do not appear to be stationary. We did a slipping window detection, which shows the average temperaures and their SD by month. They show a seasonal trend, with much lower temperatures in January than in July and also differ across multi-decade periods, suggesting a changing mean level, which is controvertial to the invariant mean value of a stationary process.\par
2. \begin{figure}[htbp]
    \centering
    \graphicspath{{png/}{STATS_Minor/701/HW1/png/}}
...
\end{figure}\par
After removing the fitted linear trend, the residuals still exhibit a seasonal pattern. With similar detection with above, the monthly mean values are mostly negative near the beginning and end of the year and positive around the middle of the year. This suggests that a linear trend does not adequately capture the seasonal mean structure and not a stationary process either.\par
...
\end{figure}\par
After removing the fitted trigonometric trend, the residuals exhibit a seasonal pattern. With similar detection with above, the monthly mean values are less than those of linear model, with more season-dependent SD. This suggests that a trigonometric trend works better on the seasonal mean structure than a linear one.
Here the displayed summaries are 30-day rolling means and standard deviations, rather than calendar-month statistics. The rolling mean stays closer to zero than in the linear-model plot, while some short-term variation remains; the plots alone do not establish stationarity. Since the two fits use different spans of data, their residual magnitudes are not directly comparable.

For the phase question in part 4, the frequency $\omega=2\pi/365$ is fixed, but the phase need not be specified in advance: for coefficients $a$ and $b$,
\[
a\cos(\omega t)+b\sin(\omega t)
=R\cos(\omega t-\phi),\qquad
R=\sqrt{a^2+b^2},\quad \phi=\operatorname{atan2}(b,a).
\]
Thus ordinary linear least squares estimates both coefficients and thereby determines the amplitude and phase (when $R>0$).
\par
\vspace{0.5em}
...
b. 1. \begin{figure}[htbp]
    \centering
    \graphicspath{{png/}{STATS_Minor/701/HW1/png/}}
    \includegraphics[width=0.8\linewidth]{HW1_5_default.png}
\end{figure}
\par
The two series look similar: both fluctuate around zero, with sample autocovariances near zero at nonzero lags.\par
...
&=\begin{cases}
\dfrac{n-1+(-1)^{h}}{n},&h\in n\mathbb{Z},\\
0,&h\notin n\mathbb{Z}.
\end{cases}
\end{aligned}
\]
...
The increasing choice emphasizes rapid fluctuations, while the oscillating choice emphasizes selected frequency bands and changes the lag pattern.\par
\begin{figure}[htbp]
    \centering
    \graphicspath{{png/}{STATS_Minor/701/HW1/png/}}

...
\]
Prove that energy estimates hold for this problem.
...
\]
We claim that $E(t)$ is constant on $[0,T]$, namely
\[
E(t)=E(0)=\frac12\int_\Omega \bigl(|u_1|^2+|\nabla u_0|^2\bigr)\,dx.
\]
Indeed, multiply the equation $\partial_t^2u-\Delta u=0$ by $\partial_tu$ and integrate over $\Omega$:
\[
...
\]
For the second term, integrating by parts in space gives
\[
-\int_\Omega \Delta u\,\partial_tu\,dx
=\int_\Omega \nabla u\cdot \nabla\partial_tu\,dx-\int_{\partial\Omega}\partial_\nu u\,\partial_tu\,dS.
\]
Because $u=0$ on $[0,T]\times\partial\Omega$, its time derivative also vanishes on the lateral boundary, so $\partial_tu=0$ on $\partial\Omega$ and the boundary term is zero. Moreover,
\[
\int_\Omega \nabla u\cdot \nabla\partial_tu\,dx
...
\]
for all $t\in[0,T]$. This is the desired energy estimate. \prvd

...
\]
which means we may choose a subsequence $u_{t_n}$ such that $\lVert\partial_x^2 u(t_n,\cdot)\rVert_{L^2([-L,L])}$ is bounded by some positive constant. By Morrey's inequality, $u(t_n,\cdot),\partial_x(t_n,\cdot)$ are both compactly embedded in $\mathcal{C}^{0,1/2}([-L,L])$ and hence we may choose a further subsequence $u(t_{n_j},\cdot) \to u_{\infty}$ with $\partial_x(t_{n_j},\cdot) \to u'_{\infty}$ in $\mathcal{C}^{0,1/2}([-L,L])$ for some $u_{\infty}$ and $u'_{\infty}$. For any test function $\phi \in C^{\infty}_c([-L,L])$, we have
\[
\begin{aligned}
...
Step 3: Now let us construct a time-invariant solution $U$ by
\[U(t,x) = u_{\infty}((x)), (t,x) \in [0,1)\times[-L,L]\]
and then $U \in \mathcal{C}([0,1),W^{1,\infty}([-L,L]))$ automatically. We also need to define a truncation solution $U_T$ which is the solution for the parabolic equation with initial condition $u(T,\cdot)$.

%Step 4: Now we compute for any test function $\phi \in C^{\infty}_c([0,l)\times[-L,L])$, we have
%\[
%\begin{aligned}
    %\Big|&\int (U_{T}(0,\cdot)-U(0,\cdot))\phi(0,\cdot)\\ + &\int_{[0,l)\times[-L,L]} (U_{T}-U)\partial_t \phi - \dfrac{1}{2}\phi(U\partial_xU - U_{T}\partial_x U_T) - \dfrac{1}{2}\partial_x\phi(\partial_x U - \partial U_T)(\partial U + \partial U_T) \Big| \\
    %\leq & L_1 + L_2 +L_3
%\end{aligned}
%\]
%where
%\[
%\begin{aligned}
    %L_1 &= \int |(U_{T}(0,\cdot)-U(0,\cdot))\phi(0,\cdot)| + \int_{[0,l)\times[-L,L]} |(U_{T}-U)\partial_t \phi|, \\
    %L_2 &= \dfrac{1}{2}\int_{[0,l)\times[-L,L]}|\phi(U\partial_xU - U_{T}\partial_x U_T)| \\
    %L_3 &= \dfrac{1}{2}\int_{[0,l)\times[-L,L]}|\partial_x\phi(\partial_x U - \partial U_T)(\partial U + \partial U_T)| \\
%\end{aligned}
%\]
%Reapplying the strong maximum and minimum principle to $U$ and we will know $|U_T - U|_u \to 0$, so $L_1$ is going to $0$ and 
%\[
%\begin{aligned}
%2L_2 &\leq \sup |\phi|\int_{[0,l)\times[-L,L]}|U-U_T||\partial_x U| + |U_T||\partial_x U - \partial_x U_T| \\
%&\leq\sup |\phi||U-U_T|_u(2Ll)^2 \int_T^{T+l}\lVert\partial_x u\rVert_{L^2(-L,L)}
%\end{aligned}
%\]
Step 4: For any $\epsilon > 0$ we consider $|\epsilon u(t,x)| \in \epsilon[m,M]$, so
\[
|(1\pm\epsilon) u - u|_u \in \epsilon[m,M]
\]
Notice that for $u^+_{T,\epsilon} := (1+\epsilon)U_T$ and $u^-_{T,\epsilon} := (1-\epsilon U_T)$ satisfy that
\[
\partial_t u^+_{T,\epsilon} \leq \tfrac12 u^+_{T,\epsilon} \partial_x^2 u^+_{T,\epsilon},\quad \partial_t u^-_{T,\epsilon} \geq \tfrac12 u^-_{T,\epsilon} \partial_x^2 u^-_{T,\epsilon}
\]
\end{proof}
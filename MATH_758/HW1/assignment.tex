...
To make the proof more readable, I will omit some natural or less important facts or notational details during the writing. If you find it hard to follow, you can refer to the appendix after the proof, where I will explain some simple conclusions (which will be marked) more clearly. To avoid confusion, I will place the mark immediately after inline formulas and before displayed formulas.\par
I should also mention that the appendix is of course part of the assignment, so references to it are intended. Enjoy your grading!

\subsection*{Problem.1} 
Let $(X,\mathcal{B},\mu,T)$ be a m.p.s. A sub-$\sigma$-algebra $\mathcal{A}$ of $\mathcal{B}$ with $T^{-1}\mathcal{A} = \mathcal{A}$ modulo $\mu$ is called a $T$-invariant sub-$\sigma$-algebra. Show that the system $(\tilde{X},\tilde{\mathcal{B}},\tilde{T},\tilde{\mu})$ defined by\par
...
d. $\tilde{\mathcal{B}}$ is the smallest $\tilde{T}$-invariant $\sigma$-algebra for which the map $\pi:\tilde{X}\to X, x\mapsto x_0$ is measurable.\\
Show that this is an invertible m.p.s. and that $\pi$ is a factor map. Thus $\tilde{X}$ is an invertible extension of $X$.
\vspace{0.5em}\\
\textbf{Sol.} \par
...
Problem.2.\par
We only need to show the conclusion when $\alpha$ is irrational. For any open set in $[0,1)$, there exists an open interval in it with length $\epsilon > 0$. Then we consider $N = [\tfrac{1}{\epsilon}]+1$, and $I_k = [\tfrac{k}{N}, \tfrac{k+1}{N})$, $0 \leq k \leq N-1$, is a partition of $[0,1)$. Since $\alpha$ is irrational, the points $\{m\alpha\}_{1\leq m \leq N+1}$ are pairwise distinct modulo $1$, so there has to be an $I_k$ containing two of them. We may assume $p\alpha,q\alpha \in I_k$ modulo $1$ for some $k$ with $p<q$. Then $(q-p)\alpha \in [0,\tfrac{1}{N})$ or $[1-\tfrac{1}{N},1)$, and hence there always exists an integer $M$ such that $M(q-p)\alpha$ lies in the open set modulo $1$.\par
Problem.3.\par
We know that if there exists a finite $f$-invariant measure $\mu$, then we may apply Poincar\'e recurrence to it. For any $[n,n+1)$, $n\in\Z$, the points in it recur to it a.e., and hence $\mu([n,n+1)) = 0$, which implies that $\mu(\R) = 0$, a contradiction.\par

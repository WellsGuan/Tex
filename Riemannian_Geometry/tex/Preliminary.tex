...
\begin{proposition}
    If $F: N \to M$ and $G: M \to P$ are $C^{\infty}$ maps, then for any $p\in N$,
...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        \item[(b)] The pair $(M,\phi)$ is a submanifold of $N$ if $\phi$ is an injective immersion.
        \item[(c)] $\phi$ is an embedding if $\phi$ is an injective immersion which is also a homeomorphism onto $\phi(M)$, that is, $\phi$ is open with $\phi(M)$ equipped with the relative topology.

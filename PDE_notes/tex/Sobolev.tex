...
Step 2 (Estimate in $H^s(\mathbb{R}^{d-1})$ and identify the sharp exponent). For $s\in\mathbb{R}$,
\[
\| \operatorname {tr}_{\partial U}u\|_{H^s(\mathbb{R}^{d-1})} \simeq \|(1+|\xi'|^2)^{s/2}\mathcal{F}_{x'}(\operatorname {tr}_{\partial U}u)(\xi')\|_{L^2_{\xi'}}.
\]
Using the identity above, we obtain
\[
\| \operatorname {tr}_{\partial U}u\|_{H^s(\mathbb{R}^{d-1})} \lesssim \left\|(1+|\xi'|^2)^{s/2}\int_{\mathbb{R}} \tilde{\tilde{u}}(\xi',\xi_d)\,\frac{d\xi_d}{2\pi}\right\|_{L^2_{\xi'}}.
\]
Insert the factor $(1+|\xi'|^2+\xi_d^2)^{1/2}$ and apply Cauchy--Schwarz in $\xi_d$. Then
\[
\| \operatorname {tr}_{\partial U}u\|_{H^s(\mathbb{R}^{d-1})} \lesssim \sup_{\xi'\in\mathbb{R}^{d-1}}\left(\int_{\mathbb{R}} \frac{(1+|\xi'|^2)^s}{1+|\xi'|^2+\xi_d^2}\,d\xi_d\right)^{1/2} \cdot \|(1+|\xi|^2)^{1/2}\tilde{\tilde{u}}(\xi)\|_{L^2_{\xi}}.
\]
The last factor is equivalent to $\|\tilde{u}\|_{H^1(\mathbb{R}^d)}$, which is in turn bounded by $\|u\|_{H^1(\mathbb{R}_+^d)}$ by $(1)$.

It remains to understand when
\[
\sup_{\xi'\in\mathbb{R}^{d-1}} \int_{\mathbb{R}} \frac{(1+|\xi'|^2)^s}{1+|\xi'|^2+\xi_d^2}\,d\xi_d
\]
is finite. Set $A:=1+|\xi'|^2\ge 1$. Then
\[
\int_{\mathbb{R}} \frac{(1+|\xi'|^2)^s}{1+|\xi'|^2+\xi_d^2}\,d\xi_d = A^s \int_{\mathbb{R}} \frac{1}{A+\xi_d^2}\,d\xi_d.
\]
With the substitution $\xi_d=\sqrt{A}\,t$, we get
\[
\int_{\mathbb{R}} \frac{1}{A+\xi_d^2}\,d\xi_d = A^{-1/2}\int_{\mathbb{R}} \frac{1}{1+t^2}\,dt \simeq A^{-1/2}.
\]
Therefore the above quantity is comparable to
\[
A^{s-1/2} = (1+|\xi'|^2)^{s-1/2}.
\]
Hence the supremum is finite if and only if $s\le 1/2$. Taking the sharp value $s=1/2$, we obtain
\[
\| \operatorname {tr}_{\partial U}u\|_{H^{1/2}(\mathbb{R}^{d-1})} \lesssim \|\tilde{u}\|_{H^1(\mathbb{R}^d)} \lesssim \|u\|_{H^1(\mathbb{R}_+^d)}.
\]
This proves the theorem.
\end{proof}

\begin{remark}
The estimate is sharp: the computation above shows that this argument cannot work for any $s>1/2$.
\end{remark}

\subsection{Extension from the boundary}
...
\subsection{Fourier representations}
...
\subsection{The Gagliardo--Nirenberg--Sobolev inequality and the Loomis--Whitney inequality}

In a nutshell, Sobolev inequalities are a quantitative generalization of the fundamental theorem of calculus: from control of the derivative we want to control the function itself.

\begin{theorem}[Gagliardo--Nirenberg--Sobolev]
Let $d\ge 2$. For $u\in C_c^{\infty}(\mathbb{R}^d)$,
\[
\|u\|_{L^{d/(d-1)}(\mathbb{R}^d)} \le C_d\|Du\|_{L^1(\mathbb{R}^d)},
\]
where $C_d$ depends only on $d$.
\end{theorem}

\begin{remark}
The exponent $d/(d-1)$ can be recovered from scaling considerations.
\end{remark}

\begin{lemma}[Loomis--Whitney inequality]
Let $d\ge 2$. For $j=1,\dots,d$, let
\[
f_j := f_j(x^1,\dots,\widehat{x^j},\dots,x^d).
\]
Then
\[
\left\|\prod_{j=1}^d f_j\right\|_{L^1(\mathbb{R}^d)} \le \prod_{j=1}^d \|f_j\|_{L^{d-1}(\mathbb{R}^{d-1})}.
\]
\end{lemma}
\begin{proof}
...
\begin{remark}
\leavevmode\\
Applied to indicator functions of coordinate projections, the Loomis--Whitney inequality controls the measure of a set by the measures of its projections.
\end{remark}
...
Let $d\ge 2$. For $u\in C_c^1(\mathbb{R}^d)$, we have
\[
\|u\|_{L^{d/(d-1)}(\mathbb{R}^d)} \le C\|Du\|_{L^1(\mathbb{R}^d)}.
\]
\end{theorem}

\begin{proof}
For each $x\in \mathbb{R}^d$ and each coordinate direction $j$, the fundamental theorem of calculus gives
\[
u(x) = \int_{-\infty}^{x_j} \partial_{x_j}u(x_1,\dots,x_{j-1},y,x_{j+1},\dots,x_d)\,dy.
\]
Hence
\[
|u(x)| \le \int_{-\infty}^{\infty} |Du(x_1,\dots,x_{j-1},y,x_{j+1},\dots,x_d)|\,dy =: \tilde f_j(x_1,\dots,\widehat{x_j},\dots,x_d).
\]
Therefore
\[
|u(x)|^d \le \prod_{j=1}^d \tilde f_j(x).
\]
Applying the Loomis--Whitney inequality, we obtain
\[
\int_{\mathbb{R}^d} |u(x)|^{d/(d-1)}\,dx \le C \prod_{j=1}^d \|\tilde f_j\|_{L^{d-1}(\mathbb{R}^{d-1})}^{1/(d-1)},
\]
